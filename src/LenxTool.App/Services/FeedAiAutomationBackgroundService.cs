...
internal sealed partial class FeedAiAutomationBackgroundService(
    IFeedAiAutomationQueueService queue,
    FeedAiAutomationOptions options,
    ILogger<FeedAiAutomationBackgroundService> logger) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        do
        {
            try
            {
                ...
                if (attempted > 0) LogBatchCompleted(logger, attempted);
            }
    ...
    [LoggerMessage(1201, LogLevel.Debug, "Processed {AttemptedCount} Feed AI automation jobs")]
    private static partial void LogBatchCompleted(ILogger logger, int attemptedCount);

    [LoggerMessage(1202, LogLevel.Warning, "The Feed AI automation queue pass failed")]
