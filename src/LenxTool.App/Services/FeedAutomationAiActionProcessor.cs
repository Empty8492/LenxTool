...
public sealed class FeedAutomationAiActionProcessor :
    IFeedAutomationAiActionProcessor
{
    private static readonly IReadOnlyCollection<FeedAutomationActionType>
        AiActionTypes = Array.AsReadOnly<FeedAutomationActionType>(
        [
    ...
    private readonly IFeedAutomationAiActionService _aiActions;
    private readonly TimeProvider _timeProvider;
    private readonly FeedAutomationActionProcessorOptions _options;

    public FeedAutomationAiActionProcessor(
        IFeedAutomationActionQueueRepository queue,
        IFeedAutomationAiActionService aiActions,
        TimeProvider timeProvider,
        FeedAutomationActionProcessorOptions options)
    {
        ArgumentNullException.ThrowIfNull(queue);
        ArgumentNullException.ThrowIfNull(aiActions);
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Rule actions can mix feeds with different concurrency limits. A single
        // worker keeps execution within every valid per-feed limit while the
        // durable queue still preserves retries and crash recovery.
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            FeedAutomationAiActionResult result =
                await _aiActions.ExecuteAsync(
                    action,
                    cancellationToken).ConfigureAwait(false);
            switch (result)
            {
                case FeedAutomationAiActionResult.Completed:
                    await TryCompleteAsync(
                        ...
                        cancellationToken).ConfigureAwait(false);
                    break;
                case FeedAutomationAiActionResult.EntryMissing:
                    await TryCompleteAsync(
                        action,
                        ...
                        cancellationToken).ConfigureAwait(false);
                    break;
                case FeedAutomationAiActionResult.FeedUnavailable:
                    await TryCompleteAsync(
                        action,
                        FeedAutomationActionRunOutcome.Failed,
                        "POLICY_DISABLED",
