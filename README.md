...
字幕 Gate 0 和 RSS P0 已完成。P1-01～P1-06 与 P1-A 已完成本机私人已读/收藏/标签/备注、组合筛选、历史页一致入口、长文阅读位置恢复，以及 schema v7 离线资源索引、安全图片下载和本地回退；真实 WPF 控件运行测试覆盖滚动、键盘焦点、临时 SQLite 重建恢复、离线缓存图片显示和失败占位。`https://daily.juya.uk/rss.xml` 仍作为兼容种子在空目录的新建 Feed 表单中预填。当前下一项是 P1-07 全文提取契约与实现评估；其他主要未完成项包括后续全文/AI/自动化能力和 JSON 双栏 Diff。完整状态、运行配置和发布阻塞项以 [`docs/PROJECT_GUIDE.md` 第 10 节](docs/PROJECT_GUIDE.md#10-当前版本边界与交付状态)为准。
