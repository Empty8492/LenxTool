...
1. 准备剩余受控输入：可写 Readeck 测试实例与 Bearer token，以及可写 Outline 测试实例、个人 API key 和专用 collection UUID。秘密只通过密码管理器或运行时安全输入提供，不写入仓库、Issue、日志或聊天。
2. 生产 D1、Worker v2、首管理员、强 ETag、schema v2/旧客户端矩阵和 Desktop v2/qBittorrent/Webhook canary 已完成；每轮继续从版本 16 的九类全禁用基线发布单 provider 最小权限，结束后立即回滚并清理 marker/测试对象。
3. 下一轮先执行 Readeck 的标签查找、首次创建、重放、归档、暂时故障与撤权，再执行 Outline 的 collection 身份、个人草稿、重复更新、目标切换、暂时故障与撤权。每个用例只记录脱敏结果、队列终态和第三方非秘密对象标识，不记录秘密或正文。
4. Readeck/Outline 关闭当前四-provider 队列后，仍要补 Eagle 独立资源库、Zotero 个人库和 Readwise Reader 的受控真实矩阵。全仓 formatter 历史基线已关闭；全部外部适配器通过后，再按 [`构建、签名与发布指南`](docs/RELEASE_GUIDE.md)重新生成并验收正式制品。
...
当前仍开放的是 Readeck、Outline、Eagle、Zotero、Readwise 真实验收，对应 provider 凭据/测试环境、签名证书/离线更新私钥和跨物理机升级矩阵；这些输入确认前，不宣称端到端生产验收完成。
