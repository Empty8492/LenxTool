...
状态核对日期：2026-07-24。当前源码已经具备资讯刷新与缓存、13 个可多选筛选的分组热点平台、可用的 Groq/DeepSeek Key 加密保存、AI 解读/趋势报告、Groq/本地 Whisper 转写、完整字幕翻译/导出闭环、文档与数据工具、全局搜索、数据库备份恢复和签名更新检查等预览能力。

字幕 Gate 0 和 RSS P0 已完成。P1-01～P1-07 与 P1-A 已完成本机私人已读/收藏/标签/备注、组合筛选、历史页一致入口、长文阅读位置恢复、schema v7 离线资源索引、安全图片下载和本地回退，以及受控全文下载、编码识别、正文块提取与白名单净化；真实 WPF 控件运行测试覆盖滚动、键盘焦点、临时 SQLite 重建恢复、离线缓存图片显示和失败占位。`https://daily.juya.uk/rss.xml` 仍作为兼容种子在空目录的新建 Feed 表单中预填。当前下一项是 P1-08 按 Feed 控制的全文抓取队列；其他主要未完成项包括后续全文阅读 UI、AI/自动化能力和 JSON 双栏 Diff。完整状态、运行配置和发布阻塞项以 [`docs/PROJECT_GUIDE.md` 第 10 节](docs/PROJECT_GUIDE.md#10-当前版本边界与交付状态)为准。
