...
2. 按 `D1 migration 0011 → Worker v2 → Desktop v2` 部署；记录迁移前 Time Travel 书签、Worker 版本、schema v2 响应与 v2 ETag，确认旧客户端兼容投影和带高级约束时的升级拒绝。
3. 逐 provider 执行最小真实矩阵：Readeck 标签查找/创建/重放，Outline collection 与草稿回执，qBittorrent magnet/`.torrent`/200/202/失败回执，Webhook OPTIONS/HMAC/幂等 ack。每个用例记录脱敏结果、队列终态和第三方实际对象，不记录秘密或正文。
...
当前仍开放的只有 P2-D 受控实例、生产 D1/Worker 部署窗口、签名证书/离线更新私钥和跨物理机升级矩阵；这些输入确认前，不宣称端到端生产验收完成。
