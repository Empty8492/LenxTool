...
1. 准备剩余受控输入：Readeck 实例、Outline 实例与 collection，以及 Webhook 接收端。测试账号、API key 和 token 只通过密码管理器或运行时安全输入提供，不写入仓库、Issue、日志或聊天。
2. 生产 D1、Worker v2、首管理员、强 ETag、schema v2/旧客户端矩阵和 Desktop v2/qBittorrent 全部 canary 已完成；每轮继续从版本 12 的九类全禁用基线发布单 provider 最小权限，结束后立即回滚并清理 marker/测试对象。
3. 下一轮优先建立受控 Webhook 接收端并验证 OPTIONS/HMAC/幂等 ack，再执行 Readeck 标签查找/创建/重放与 Outline collection/个人草稿回执。每个用例记录脱敏结果、队列终态和第三方实际对象，不记录秘密或正文。
...
当前仍开放的是 Readeck/Outline/Webhook 真实外联、Provider key、签名证书/离线更新私钥和跨物理机升级矩阵；这些输入确认前，不宣称端到端生产验收完成。
