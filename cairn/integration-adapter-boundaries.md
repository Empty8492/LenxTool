...
- qBittorrent 真实 canary 的受控 `.torrent` 由内存 metainfo 和测试 fetcher 交付给生产 exporter/client；这能证明真实文件上传、provider 回执和写后复查，但不能替代生产 `TorrentFileFetcher` 的公网 HTTPS/DNS/MIME/bencode 连通。公共 exporter 不暴露原始 HTTP 状态时，也不能从成功结果倒推出 200 或 202；状态矩阵必须另建不泄密观测。
