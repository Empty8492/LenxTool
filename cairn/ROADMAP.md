...
- [x] 完成生产 schema v2 GET/PUT、强 ETag/条件写、幂等/冲突和旧客户端兼容矩阵；契约检查点的版本 2 为九类全禁用基线，当前经 canary 回滚后为版本 4 全禁用。
...
1. Readeck、Outline、Webhook 使用哪些受控实例与测试账号；qBittorrent 如何提供可公开 HTTPS 获取的受控 `.torrent` 和状态级观测？
