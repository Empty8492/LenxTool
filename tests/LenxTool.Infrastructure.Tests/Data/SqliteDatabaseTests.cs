...
public sealed class SqliteDatabaseTests : IDisposable
{
    ...
    public async Task InitializeCreatesRequiredSchemaAndAppliesMigrationOnce()
    {
        ...
        [
            ...
            "entry_export_tasks", "local_scheduled_tasks",
            "local_schedule_runs"
        ];
        ...
        Assert.Contains("ix_local_scheduled_tasks_due", indexes);
        Assert.Contains("ix_local_schedule_runs_claim", indexes);
        Assert.Contains("ix_local_schedule_runs_history", indexes);
    ...
    [Fact]
    public async Task SchemaVersionTwentyTwoUpgradeAddsRunWindowsAndPreservesSchedule()
    {
        LocalScheduledTask expected;
        using (SqliteDatabase versionTwentyTwo = CreateDatabase())
        {
            await versionTwentyTwo.InitializeAsync(CancellationToken.None);
            expected = await new LocalScheduledTaskRepository(versionTwentyTwo)
                .SaveAsync(
            ...
            await using SqliteConnection connection =
                await versionTwentyTwo.OpenConnectionAsync(
                    CancellationToken.None);
        ...
        Assert.Equal(
            expected,
            await new LocalScheduledTaskRepository(upgraded).GetAsync(
                expected.Id,
                CancellationToken.None));
        ...
        check.CommandText = """
            SELECT COUNT(*)
            FROM sqlite_master
            WHERE type='table' AND name='local_schedule_runs';
            """;
        Assert.Equal(
            1L,
    ...
    public async Task SchemaVersionSevenUpgradeAddsDefaultFullTextPolicyAndPreservesEntries()
    {
        using (SqliteDatabase versionSeven = CreateDatabase())
        {
            ...
            command.CommandText =
                ...
                ALTER TABLE ai_reports DROP COLUMN target_language;
                ALTER TABLE ai_reports DROP COLUMN prompt_version;
                ALTER TABLE ai_reports DROP COLUMN prompt_tokens;
                ALTER TABLE ai_reports DROP COLUMN completion_tokens;
                ALTER TABLE ai_reports DROP COLUMN duration_ms;
                ALTER TABLE ai_reports DROP COLUMN error_code;
                ALTER TABLE ai_reports DROP COLUMN updated_at;
                ...
                DROP TABLE feed_media_deliveries;
