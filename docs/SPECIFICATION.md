...
- Inno Setup 生成安装程序；`dotnet publish` 生成自包含 .NET 的 win-x64 应用与便携 ZIP。Windows App SDK Runtime 由安装器提供，便携版缺失时只降级系统通知。
...
- 图片、音频和视频使用独立惰性视图并复用分类查询与本地筛选；音频选择不得自动访问来源，显式播放支持本地断点；视频不嵌入网页播放器，大型/未知下载必须显示大小、固定目标和空间并再次确认。音视频投递复用安全幂等媒体链，视频还需本机兼容性验证；不支持或断流时只允许二次确认后打开安全原文。
...
- Windows 系统通知必须默认关闭并保留应用内收件箱为耐久真相；启用后只允许通用提示或标题，不发送正文/来源。激活参数只携带本地通知 ID，应用重读 SQLite 后才能导航到封闭本地实体，不接受任意 URI；Runtime 或系统能力缺失不得阻止主程序启动。
