...
`ToolsViewModel` 在 UI 线程捕获左右输入快照，并把校验和结构比较移到后台线程。Core 的 `JsonToolkit.Diff` 接受取消令牌与最大差异数；对象和数组递归均检查取消，达到 500 项后仅设置截断标志，不继续构造无界结果。每侧输入在进入解析前限制为 2 MiB，单个值预览限制为 2,048 个字符，列表启用回收虚拟化。

每次比较同时捕获单调任务代际和输入修订。编辑、交换或显式取消会推进相应计数并取消令牌；后台返回 UI 后再次检查令牌、代际、修订和快照，因此即使任务已经完成但 continuation 尚未恢复，或输入发生 A→B→A 回绕，旧结果也不能发布。
