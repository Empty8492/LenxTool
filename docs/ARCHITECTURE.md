...
`ToolsViewModel` 在 UI 线程捕获左右输入快照，并把校验和结构比较移到后台线程。Core 的 `JsonToolkit.AnalyzeDiffAsync` 对每侧只构造一次解析树，同时返回两侧独立校验结果与结构差异；严格 UTF-8 编码前后检查取消，异步解析流把单次读取限制为 64 KiB 并在每块之间协作取消。对象和数组遍历继续传播同一取消令牌；结果达到 500 项，或已渲染路径累计达到 256 KiB 字符预算时设置截断标志并停止收集。属性路径使用 JSON 字符串转义的方括号表示法，使 `$["a.b"]` 与 `$["a"]["b"]` 保持无歧义；单路径最多 1,024 个字符，超长链路以确定性的 `path-sha256` 标识代替，正常短路径不计算摘要。每侧输入在进入解析前限制为 2 MiB，单个值预览限制为 2,048 个字符；生产滚动区给差异列表提供有限视口并启用回收虚拟化，500 行结果不会一次实现全部容器。

每次比较同时捕获单调任务代际和输入修订。编辑、交换或显式取消会推进相应计数并取消令牌；后台返回 UI 后再次检查令牌、代际、修订和快照，因此即使任务已经完成但 continuation 尚未恢复，或输入发生 A→B→A 回绕，旧结果也不能发布。JSON Diff 页签自带生产滚动区；真实 WPF 回归直接运行最小 920×620 的 `MainWindow`，并在同一宿主验证等效 200% 布局下右栏和差异列表仍可达。
