...
- 资讯筛选下拉框按 `Label` 正确显示，不再泄漏筛选对象文本；空状态完整覆盖列表与阅读器。所有原生滚动区采用参考 `TwilightLemon/FluentScrollViewer` 参数的统一滚轮模型：标准鼠标累加速度并指数衰减，高分辨率触控板连续逼近累计目标，同时尊重 Windows 滚轮行数设置。普通页面先提交解析落点，再以 `RenderTransform` 在合成阶段推进视觉位置，避免复杂页面逐帧布局。像素虚拟列表使用 Recycling 并在前后各保留一屏容器，按滚动方向校验缓存覆盖距离，越过缓存时安全回退连续帧偏移。“减少动画”直接提交同一落点而不降低距离，切换一级资讯入口时仍自动回到顶部。
...
- 全局滚动体验已补强：类级事件处理覆盖显式与控件模板内部的 `ScrollViewer`，嵌套滚动区优先消费最内层仍可滚动的容器，到达边界后交还外层。标准鼠标按 `TwilightLemon/FluentScrollViewer` 的速度倍率、摩擦系数和 144 Hz 时间基准形成可叠加惯性，高分辨率触控板使用独立精确模式；LenxTool 以解析积分稳定不同刷新率下的最终距离，并补充系统行数、反向/边界、直接输入接管和“减少动画”处理。热点及每日早报等重页面仍以一次逻辑落点配合合成层 `RenderTransform` 推进视觉位置，不会每帧调用 `ScrollToVerticalOffset`。ListBox、ListView、PagedListBox 与下拉列表使用 Recycling 像素虚拟化、前后各一屏容器缓存和同一合成路径；合成前会按滚动方向校验目标距离、缓存单位及变换所有权，越过缓存或内容已有外部变换时回退连续帧偏移。每日早报只实现当前视口前后一屏的正文块，统一由每个祖先滚动区的单一协调器刷新；合成滚动把全文视口扫描按累计安全位移合并，离开缓冲区的图片立即取消后台下载与解码。热点平台卡只实现当前视口与半屏缓冲区。快速连续滚动复用单一状态，反向输入、拖动、键盘、触控、程序化定位、回顶和卸载会安全接管当前位置；每次实窗动画会记录平均 FPS、P95 帧间隔、最差帧和长帧数。横向滚动保持原生行为，回顶改为立即定位。
- 滚动算法来源边界：仅参考 [`TwilightLemon/FluentScrollViewer` 提交 `63f07a9`](https://github.com/TwilightLemon/FluentScrollViewer/blob/63f07a972bfde3d9a517f5c0f13f105df5a64b34/MyScrollViewer.cs) 公开展示的速度倍率、摩擦/插值参数、144 Hz 时间基准和输入分类思路；未复制其 `MyScrollViewer`、`StylusTouchDevice`、XAML 或事件控制结构。LenxTool 独立实现了解析积分、系统滚轮设置、嵌套路由、反向接管、减少动画、虚拟缓存和重页面合成层交接，因此不引入上游源码或二进制依赖。
