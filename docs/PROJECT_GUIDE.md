...
- 资讯筛选下拉框按 `Label` 正确显示，不再泄漏筛选对象文本；空状态完整覆盖列表与阅读器。所有原生滚动区统一使用每日早报的 1.45 倍滚轮灵敏度和 160～220 ms 平滑过渡；普通页面先提交一次逻辑位置，再以 `RenderTransform` 在合成阶段完成视觉补间，避免复杂页面逐帧布局。像素虚拟列表使用 Recycling 并在前后各保留一屏容器，目标位于缓存覆盖距离内时使用同一合成路径，连续输入越过缓存时安全回退连续帧偏移。“减少动画”只关闭过渡而不降低距离，切换一级资讯入口时仍自动回到顶部。
...
- 全局滚动体验已补强：类级事件处理覆盖显式与控件模板内部的 `ScrollViewer`，嵌套滚动区优先消费最内层仍可滚动的容器，到达边界后交还外层。普通页面和“回到顶部”都以一次逻辑滚动配合合成层 `RenderTransform` 完成过渡，热点类重页面不再每帧调用 `ScrollToVerticalOffset`。ListBox、ListView、PagedListBox 与下拉列表使用 Recycling 像素虚拟化、前后各一屏容器缓存和同一合成式补间；合成前会校验实际目标距离、缓存单位及变换所有权，越过缓存或内容已有外部变换时退回连续帧偏移。每日早报只实现当前视口前后一屏的正文块，统一由每个祖先滚动区的单一协调器刷新；合成式回顶期间也会随可见画面换入/换出内容，离开缓冲区的图片立即取消后台下载与解码。热点平台卡只实现当前视口与半屏缓冲区。快速连续滚动复用单一状态，反向输入、拖动、键盘、触控和程序化定位会安全接管当前位置；每次实窗动画会记录平均 FPS、P95 帧间隔、最差帧和长帧数，60/120Hz 统计由纯逻辑测试校准，未提供真实刷新率时只报告推断节奏、不宣称达到帧预算。横向滚动和系统“减少动画”语义保持原生行为。
...
Gate 0 字幕闭环、P0“管理员策展 RSS”、P1“阅读增强、AI 与自动化”、P2-01～P2-13，以及插入计划 DISC-01～DISC-06、UX-03 均已完成。[P2 内容视图与集成计划](plans/RSS_P2_VIEWS_INTEGRATIONS.md) 的下一个窄切片为 P2-14 Readwise 适配器；P2-15 及以后仍需逐项明确选择。P1/P2 源码进度不等于正式签名发布完成。
...
4. P2-01～P2-13 已完成五视图、智能视图、统一导出契约、安全集成策略、持久化队列、本地 Markdown、受控 Obsidian Vault、Eagle 图片与 Zotero 个人库导出；[`plans/RSS_DISCOVERY_AND_CONTROL_UX.md`](plans/RSS_DISCOVERY_AND_CONTROL_UX.md) 的 DISC-01～DISC-06 已交付独立发现索引、安全可替换 provider、管理员统一发现页、确认发布闭环和最终检查点，UX-03 已交付原生 WPF 共享控件模板，不依赖 Folo 私有 API 或复制其源码。
5. P2-14 及以后桌面/外部导出适配器、本地定时摘要和 Windows 系统通知仍需逐项推进；具体见 [`plans/RSS_P2_VIEWS_INTEGRATIONS.md`](plans/RSS_P2_VIEWS_INTEGRATIONS.md)。
