...
- 已完成可取消、可重试、可恢复的字幕批量翻译契约；下一步实现 DeepSeek 适配器并记录模型/token 用量。
- 保持原序号和时间轴，导出译文 SRT、双语 SRT 与 TXT，并提供打开输出目录操作。
- 将字幕片段和翻译结果写入 SQLite，支持历史查看与后续重新导出。

当前 `SrtCodec` 已具备严格 SRT 解析、原文/双语 SRT 和纯文本编码能力，字幕片段事务仓储、已有 SRT 导入和批量翻译契约已经完成；媒体转写仍只自动导出原文 SRT，尚缺转写结果写入仓储、DeepSeek 翻译实现、译文导出交互和完整工作流接线。
