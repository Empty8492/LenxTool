...
- P1-03 时间线编辑与筛选切片已完成：Feed 阅读器批量水合通用收藏，并提供收藏切换、4000 字本机备注、80 字标签创建/关联/移除、保存/失败状态和选择代次保护；时间线可组合筛选未读/已读、收藏与标签，条件在 SQLite 分页前执行，收藏兼容 favorites 与旧 `user_entry_states.is_starred`。打开即标记已读、取消备注、历史页入口和真实 WPF 验收仍待完成。
