...
- Gate0-04a 已实现 DeepSeek 字幕翻译基础适配器：从 DPAPI 密钥仓储读取 Key，按字符上限分批，只发送序号和原文；严格校验缺项、增项与重复项，按输入顺序返回译文，并携带模型、token 和精确恢复点。假 HTTP 已覆盖成功、乱序、缺项、取消和恢复；尚未接入依赖注入与界面，429/超时重试仍待 Gate0-04b。
...
- 已完成字幕批量翻译契约和 Gate0-04a 基础 DeepSeek 适配器；下一步在 Gate0-04b 补齐 429/超时重试与依赖注入接线。
- 保持原序号和时间轴，导出译文 SRT、双语 SRT 与 TXT，并提供打开输出目录操作。
- 将字幕片段和翻译结果写入 SQLite，支持历史查看与后续重新导出。

当前 `SrtCodec` 已具备严格 SRT 解析、原文/双语 SRT 和纯文本编码能力，字幕片段事务仓储、已有 SRT 导入、批量翻译契约及基础 DeepSeek 适配器已经完成；媒体转写仍只自动导出原文 SRT，尚缺转写结果写入仓储、翻译重试与界面接线、译文导出交互和完整工作流接线。
...
- 字幕片段仓储和已有 SRT 导入写入已经完成，但转写结果入库、历史检索和模型/token 用量展示尚未完成。
