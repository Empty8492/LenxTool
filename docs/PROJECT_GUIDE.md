...
- JSON：格式化、压缩、语法校验、属性排序，以及双栏结构 Diff；Diff 支持独立错误定位、交换、取消、2 MiB 单侧上限和 500 项显示上限。
- Base64 和 URL 编解码。
- 文本重复行删除、空白归一和空行折叠。
- SHA-256 文件校验，支持取消。
...
6. Independent-01 JSON 双栏结构 Diff 已完成；有界/可取消 Core 算法、双栏 UI、交换、虚拟化差异列表和真实 WPF 运行时回归均已接线，未修改 RSS 模型、SQLite 或 Worker。
