...
Gate 0 字幕闭环已经完成。P0“管理员策展 RSS”已完成服务端契约、身份生命周期、目录 schema、管理员 CRUD、只读目录、桌面安全会话、账号 UI、本地 Feed schema、目录原子仓储、自动同步、安全发现、统一解析、条件调度抓取和条目仓储/FTS（P0-01～P0-14/P0-B）。进入 P0-D 前先补齐 P0-C“至少 20 个真实 RSS/Atom fixture”的检查点；随后实现 P0-15 管理员订阅管理页。
...
2. P0-01～P0-14 与 P0-B 已完成；下一步补齐 P0-C fixture 检查点，再实现管理员管理页、OPML、时间线和首页真实数据；具体见 [`plans/RSS_P0_ADMIN_CATALOG.md`](plans/RSS_P0_ADMIN_CATALOG.md)。
...
总路线、参考项目和许可证边界见 [`plans/RSS_MASTER_ROADMAP.md`](plans/RSS_MASTER_ROADMAP.md)，架构决策见 [`decisions/ADR-001-admin-curated-rss.md`](decisions/ADR-001-admin-curated-rss.md)。只有上述 P0-01～P0-14 可作为已实现基础；20 fixture 检查点、真实管理/阅读界面和后续 P1/P2 仍不能作为已交付功能宣传。
...
- 管理员分类/Feed 写 API、普通用户只读目录、ETag/304、桌面角色可见性和本地目录自动同步已实现；安全发现/抓取与管理交互仍未实现。
- 安全 Feed URL 发现已实现；通用条目解析、抓取调度、OPML、时间线、Feed 健康、全文/图片离线、自动化规则和外部导出仍仅有详细计划。
