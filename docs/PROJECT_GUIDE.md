...
Gate 0 字幕闭环、P0“管理员策展 RSS”、P1“阅读增强、AI 与自动化”、P2-01～P2-14，以及插入计划 DISC-01～DISC-06、UX-03 均已完成。[P2 内容视图与集成计划](plans/RSS_P2_VIEWS_INTEGRATIONS.md) 的 P2-15 Cubox 因与既有导出能力重叠、官方幂等与安全重试契约不足而取消实施；客户端不保存 Cubox API 凭据，也不注册连接探针或导出器。当前窄切片为 P2-20 本地定时任务：once/daily/weekly/monthly 的本地时区换算、schema v22 计划定义持久化，以及 schema v23 的 RunOnce/Skip 漏跑语义、唯一窗口领取、租约过期接管和陈旧持有者拒绝已经完成。下一片是后台处理器、禁用与在途窗口的取消协同、生产 DI 和用户界面；目前只有持久化执行契约，定时摘要尚不可用。P1/P2 源码进度不等于正式签名发布完成。
...
5. P2-15 Cubox 已取消实施；P2-20 已完成本地时区重复计算、schema v22 计划定义持久化/启停和 schema v23 窗口幂等/漏跑恢复契约，但后台处理、在途取消协同、生产 DI 和 UI 尚未完成。P2-16～P2-19 与 P2-21 以后仍需逐项推进，具体见 [`plans/RSS_P2_VIEWS_INTEGRATIONS.md`](plans/RSS_P2_VIEWS_INTEGRATIONS.md)。
