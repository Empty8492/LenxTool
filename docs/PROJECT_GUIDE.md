...
- P1-15 Core 确定性解释器、运行账本、动作租约与受限本地状态动作已完成：ACTIVE 规则先验证并编译为可复用只读规则集，按优先级降序、冲突顺序升序、规则 ID 和规则内动作顺序稳定计算。九类字段、ALL/ANY、带时区时间、音视频存在性和非回溯 regex 均在纯内存中匹配；加标签按本地 `NOCASE` 语义去重，其余副作用动作全局只保留第一个胜者。schema v12 在单个 SQLite 事务中保存每个条目/规则版本的命中结果和全部动作决策，计划动作使用稳定幂等键，被抑制动作保留胜者引用；同一版本重放或重启后再次暂存都不会追加记录。待执行动作按确定顺序领取，租约过期可回收，旧持有者不能提交；失败可延时重试，主动释放可立即重领，成功/永久失败进入不可再次领取的终态。schema v13 增加按本地档案隔离的隐藏状态，普通时间线默认排除隐藏条目；本地执行服务只接受加标签、隐藏和标为已读，先做有界载荷校验，再调用原子追加标签或私人状态仓储。追加标签保留现有标签与同名标签颜色，重放不重复关联；缺失条目返回可终结结果。AI、媒体与通知动作会在读取条目前被拒绝，等待后台处理器交给各自受限适配器，数据库事务仍不请求网络。
...
Gate 0 字幕闭环和 P0“管理员策展 RSS”已经完成。P1-01～P1-14、P1-A、P1-B 与 P1-C 已完成；P1-15 Core 确定性解释器、SQLite v12 幂等运行账本、持久动作租约和受限本地状态动作已完成。下一切片进入 P1-15 后台动作处理器和 AI/媒体受限队列适配。
...
3. P1-01～P1-14、P1-A、P1-B 与 P1-C 已完成；P1-15 Core 解释器、SQLite v12 幂等运行账本、持久动作租约和受限本地状态动作已完成，继续后台动作处理器和 AI/媒体受限队列适配，具体见 [`plans/RSS_P1_READING_INTELLIGENCE.md`](plans/RSS_P1_READING_INTELLIGENCE.md)。
...
- Feed AI 本地缓存、单条/批量摘要、可恢复条目翻译、管理员策略、共享额度门控和本地自动处理已建立；通用管理员规则的安全契约、权威版本、发布 API、本地解释器和运行账本已建立，规则动作执行器与管理 UI 仍待 P1-15/P1-16。
