...
- P1-03 时间线交互与筛选切片已完成：Feed 阅读器批量水合通用收藏，并提供收藏切换、4000 字本机备注、80 字标签创建/关联/移除、保存/取消/失败状态和选择代次保护；打开未读条目会自动写入已读，仍可手动恢复未读，动作按钮随状态显示；时间线可组合筛选未读/已读、收藏与标签，条件在 SQLite 分页前执行，收藏兼容 favorites 与旧 `user_entry_states.is_starred`。历史页入口和真实 WPF 验收仍待完成。
