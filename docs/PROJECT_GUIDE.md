...
- P1-15 Core 确定性解释器已完成：ACTIVE 规则先验证并编译为可复用只读规则集，按优先级降序、冲突顺序升序、规则 ID 和规则内动作顺序稳定计算。九类字段、ALL/ANY、带时区时间、音视频存在性和非回溯 regex 均在纯内存中匹配；加标签按本地 `NOCASE` 语义去重，其余副作用动作全局只保留第一个胜者。计划同时保留未命中/停用规则和被抑制动作、胜出规则版本及动作顺序，供下一片写入 SQLite 运行记录；本片不写数据库、不入队、不请求网络。
...
Gate 0 字幕闭环和 P0“管理员策展 RSS”已经完成。P1-01～P1-14、P1-A、P1-B 与 P1-C 已完成；P1-15 Core 确定性解释器已完成。下一切片进入 P1-15 SQLite 幂等运行记录与受限动作入队。
...
3. P1-01～P1-14、P1-A、P1-B 与 P1-C 已完成；P1-15 Core 解释器已完成，继续 SQLite 幂等运行记录与受限动作入队，具体见 [`plans/RSS_P1_READING_INTELLIGENCE.md`](plans/RSS_P1_READING_INTELLIGENCE.md)。
