...
- Gate0-04 已完成 DeepSeek 字幕翻译适配器：从 DPAPI 密钥仓储读取 Key，按字符上限分批，只发送序号和原文；严格校验缺项、增项与重复项，按输入顺序返回译文，并携带模型、token、实际请求数和精确恢复点。每批最多请求 3 次，429 尊重受限 `Retry-After`，超时短退避，取消不重试；适配器已注册为 `ISubtitleTranslator`，假 HTTP 覆盖成功、乱序、缺项、取消、429、超时、畸形等待头和部分批次恢复，尚未接入媒体工作台界面。

### 10.2 下一里程碑

下一里程碑是补齐媒体工作台的字幕交付闭环：

- 已完成已有 SRT 导入；仍需让新完成的转写片段直接写入同一字幕仓储。
- 已完成字幕批量翻译契约、可重试 DeepSeek 适配器及依赖注入接线；下一步在 Gate0-05 接入媒体工作台的翻译进度、取消/恢复和译文导出交互。
- 保持原序号和时间轴，导出译文 SRT、双语 SRT 与 TXT，并提供打开输出目录操作。
- 将字幕片段和翻译结果写入 SQLite，支持历史查看与后续重新导出。

当前 `SrtCodec` 已具备严格 SRT 解析、原文/双语 SRT 和纯文本编码能力，字幕片段事务仓储、已有 SRT 导入、批量翻译契约、可重试 DeepSeek 适配器及依赖注入注册已经完成；媒体转写仍只自动导出原文 SRT，尚缺转写结果写入仓储、翻译界面接线、译文导出交互和完整工作流接线。
...
- 字幕片段仓储、已有 SRT 导入和可恢复 DeepSeek 批量翻译服务已经完成，但转写结果入库、翻译/导出界面、历史检索和模型/token 用量展示尚未完成。
