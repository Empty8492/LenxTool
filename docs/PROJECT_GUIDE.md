...
当前 `SrtCodec` 已具备 SRT 解析、原文/双语 SRT 和纯文本编码能力，媒体任务也已具备转写与原文 SRT 导出，字幕片段事务仓储已经完成；尚缺 SRT 导入、转写/导入结果写入仓储、翻译服务、译文导出交互和完整工作流接线。
...
- 字幕片段仓储虽已完成，但转写/SRT 导入写入接线、历史检索和模型/token 用量展示尚未完成。
