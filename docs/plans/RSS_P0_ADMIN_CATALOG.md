...
状态：进行中，P0-01～P0-14 与检查点 P0-B/P0-C 已完成
最后核对：2026-07-22
...
**完成记录（2026-07-22）：** 设置页已接入真实账号状态、登录/退出、过期提示、角色与 AI/语音额度；侧栏状态随会话更新，管理员入口只在当前服务端会话角色为 admin 时可见，角色降级会立即离开管理员页。入口当前明确为 P0-15 管理界面的占位，不把客户端隐藏当作授权。App 新增 7 项账号/导航/XAML 测试；最新 Release 程序集已完成真实 WPF 启动、未配置状态、控件自动化名称、焦点和禁用按钮键盘跳过验收。
