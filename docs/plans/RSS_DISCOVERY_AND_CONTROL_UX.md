...
- [ ] URL、RSSHub 路由、关键词和非法输入有确定分类。
- [ ] 候选项携带规范化标识、来源、置信度、健康和警告，不用显示文本驱动业务逻辑。
- [ ] 合并去重以规范化 Feed URL 为主键，来源证据可保留。

**验证：** 参数化单测覆盖 Unicode 关键词、大小写、尾斜杠、重定向后地址、危险协议和超长输入。
...
- [ ] 索引只包含允许上云的 Feed 元数据，不含条目正文或私人状态。
- [ ] 查询有长度、分页和速率限制；排名稳定并返回来源证据。
- [ ] 普通用户可按产品策略读取结果但不能写目录，管理员写入仍走原目录端点。
- [ ] 旧数据库可原位迁移并由现有 `managed_feeds` 安全回填。

**验证：** 真实 workerd/D1 覆盖迁移、分页、限流、RBAC、空结果和敏感字段不存在。
...
- [ ] 直接 URL 复用现有 SSRF、固定地址、重定向、大小和超时限制。
- [ ] 提供方具备独立超时、并发上限、缓存、熔断和脱敏错误。
- [ ] RSSHub/平台适配器只有在官方条款与稳定接口核对完成后才启用。
- [ ] 一个提供方失败时仍返回其他候选，并明确部分结果状态。

**验证：** 覆盖超时、429、畸形响应、重复候选、取消、私网/回环/重绑定与全部来源不可用。
...
- [ ] 输入会显示识别类型，支持提交、取消和防抖，不因旧请求覆盖新结果。
- [ ] 候选卡片显示来源、健康、警告和最多 4 条真实近期预览。
- [ ] 加载、部分成功、无结果、离线和限流状态可区分且可重试。
- [ ] 普通用户不显示或不可进入管理写入口；绕过 UI 仍无法发布。
