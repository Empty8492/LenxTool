...
状态：已完成；DISC-01～DISC-06、UX-03 已全部验收，后续 P2-03～P2-04 已完成并进入 P2-05
...
- [x] 发布前显示规范化地址、分类、刷新和视图策略，管理员必须确认。
- [x] 重复 Feed 转为“查看现有项”；并发冲突刷新目录且不自动重放写入。
- [x] 使用现有 `If-Match`、幂等键、RBAC 和审计；普通用户写入得到 403。
- [x] 发布成功后目录版本刷新，发现结果状态同步更新。

**验证：** 覆盖重复提交、双管理员冲突、令牌过期、普通用户、网络中断和成功后刷新。

**完成记录（2026-07-28）：** 发现候选现在以管理员 ALL 快照按规范化 URL 标记新增或现有项；现有项只显示其分类、刷新、视图和全文策略，不触发目录写入。新增候选进入独立确认面板，完整显示规范化 HTTPS 地址和四类策略，任一策略变化都会撤销确认，只有管理员再次勾选后才能提交。写入直接复用 P0 的目录管理员服务，因此保留 `If-Match`、同次 401 刷新复用幂等键、Worker RBAC/审计和服务端 URL 复核；命令运行中阻止快速双击重复提交。`CATALOG_VERSION_CONFLICT` 只刷新 ALL 目录且不重放；网络中断、取消或写后刷新失败会把结果标为未知并锁住后续发布，直到只读刷新确认。成功刷新达到返回版本后，候选立即转成“查看现有项”。新增 7 项 App 行为测试，既有 Infrastructure 目录传输测试继续覆盖令牌过期、普通用户 403、幂等键和单次冲突；App 全量为 234/234，最终全仓数字由 DISC-06 检查点确认。
...
- [x] .NET Release 构建/测试和 Worker strict typecheck/test 全部通过。
- [x] 关键词首批结果、直接 URL 探测和候选预览在约定预算内；慢源不拖垮整体。
- [x] D1 无正文/私人状态，日志无完整令牌、敏感查询参数或本地路径。
- [x] 普通用户写入 403、管理员并发冲突、离线降级和恶意 URL 有真实证据。
- [x] 用户指南、项目指南、威胁模型和测试报告同步。

**验证：** 真实 SQLite + workerd/D1 + Release WPF 联合检查；完成后把本计划状态改为已完成，再从 P2-03 继续。

**完成记录（2026-07-28）：** 默认预算已由测试锁定为已知目录关键词来源 8 秒、直接 URL 安全探测 20 秒；每个 provider 使用独立超时、并发门闩、缓存和熔断。真实 workerd/D1 在 5,000 条共享 Feed 容量上限下返回首批 50 条关键词结果的完整测试用例耗时 188 ms；真实 schema v18 SQLite 在 100 个候选、2,500 条含大正文的本地条目上一次投影 400 条预览实测 55 ms，并由 2 秒预算断言保护。慢 provider 超时仍返回健康来源的部分结果，发现页对全源不可用显示可重试离线状态。Worker 响应字段白名单、D1 schema/哨兵测试和发现链路静态审计确认云端不保存正文或私人状态，发现路径没有日志调用，也不把令牌、查询值或本地路径写入日志；原始 `request.url` 只用于 2 KiB 长度检查，解析后的查询仅进入参数化 SQL 和规范化响应。真实 workerd/D1 与桌面传输测试继续覆盖普通用户写入 403、同版本并发单赢家、409 不自动重放；安全探测覆盖私网/环回/混合 DNS/重绑定、恶意重定向、XXE、压缩炸弹和超限响应。最终门禁为 .NET Release 755/755（Core 140、Infrastructure 380、App/WPF 235）、Worker 64/64、strict typecheck、NuGet/npm 0 漏洞及构建 0 警告/0 错误；用户指南、项目指南、威胁模型、测试报告与主路线状态已同步。
