...
| 现有未完成项 | 当前事实 | 处理方式 | 归属 |
|---|---|---|---|
| SRT 导入、批量翻译、双语/译文导出 | `SrtCodec` 和原文导出已有；交互、翻译、片段持久化未接线 | 先收口当前里程碑，再让 Feed 媒体复用它 | Gate 0 |
...
| 字幕片段/模型用量历史 | 表和部分模型字段已有，仓储/UI 不完整 | Gate 0 持久化，P1 统一搜索 | Gate 0 / P1-14 |
...
- [ ] 一次事务替换指定任务的片段，序号与时间轴唯一。
- [ ] 可读取原文、译文和置信指标，重启后数据不丢失。
- [ ] 失败写入完整回滚，不留下半批片段。

**验证：** 临时 SQLite 往返、覆盖写入、回滚测试；Release 构建。
...
- [ ] 支持 UTF-8/常见 BOM，错误行给出可操作提示。
- [ ] 导入后片段立即持久化，关闭重开仍可选择。
- [ ] 中文、空格和长路径可用。

**验证：** 黄金 SRT、畸形 SRT、真实文件选择手测。
...
- 现有 85 个 .NET 测试和 Worker typecheck/test 是回归基线，不是 P0/P1/P2 的充分验收。
