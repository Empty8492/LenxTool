...
状态：实施中（Gate0-01～03、Gate0-04a 已完成；Gate0-04b～06 尚未完成）
最后核对：2026-07-21
...
| 现有未完成项 | 当前事实 | 处理方式 | 归属 |
|---|---|---|---|
| SRT 导入、批量翻译、双语/译文导出 | 严格 `SrtCodec`、事务仓储、已有 SRT 导入和基础 DeepSeek 批次适配器已完成；重试、转写入库与导出交互未接线 | 先收口当前里程碑，再让 Feed 媒体复用它 | Gate 0 |
...
**验证：** Gate0-04a 于 2026-07-21 通过 4 项假 HTTP 测试：成功与 token 解析、乱序输出按输入序号恢复、缺项结构化失败、取消和精确断点恢复。父项保持未完成；Gate0-04b 仍需覆盖 429、超时重试和依赖注入接线。
