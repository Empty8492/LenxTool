...
测试日期：自动化门禁 2026-08-17、qBittorrent 与 Webhook 生产验收 2026-08-18（Asia/Shanghai）
版本：0.1.0  
...
| 测试组 | 结果 |
|---|---:|
| LenxTool.Core.Tests（2026-08-17） | 222 passed |
...
2026-08-18 qBittorrent 公网矩阵收口后运行了与本轮范围相称的新鲜门禁：Infrastructure 的 `ManagedIntegrationApiClientTests`、`ManagedEntryExporterTests`、`EntryIntegrationServicesTests` 合计 49/49，Core 的 `EntryIntegrationPolicyTests` 36/36；整个 `LenxTool.slnx` Release 构建 0 警告/0 错误，`dotnet format --verify-no-changes`、12 个变更 Markdown 的相对链接检查、tracked diff 秘密模式扫描和 `git diff --check` 均通过。本轮没有修改产品代码，也没有把 2026-08-17 的完整测试数字冒充为当日重跑。

同日 Webhook 生产矩阵开始前重新运行了范围门禁：Infrastructure 的 Webhook/集成健康聚焦 22/22、App Webhook 聚焦 2/2、Core 策略 36/36、Worker 82/82、strict typecheck、临时接收器契约 8/8、验收工具 Release build 0 警告/0 错误均通过。收口后再次确认整个 `LenxTool.slnx` Release build 0 警告/0 错误、`dotnet format --verify-no-changes`、13 个变更 Markdown 的相对链接、tracked diff 秘密模式扫描与 `git diff --check` 均通过。验收工具与接收器位于忽略的 `artifacts/`，用于一次性取证，不作为产品源码或发布制品提交。
...
formatter 修复后的新鲜门禁为 Core 222/222、Infrastructure 811/811、App 非 WPF 523/523、10 个 WPF runtime 类逐进程 14/14、Release build 0 警告/0 错误、Worker 82/82、strict typecheck、11 个 migration LF-only、NuGet/npm 漏洞审计 0、`git diff --check` 与 `dotnet format --verify-no-changes` 通过。2026-08-18 又完成 qBittorrent 剩余真实公网/状态矩阵和 Webhook 真实公网矩阵；Readeck、Outline 仍开放，Eagle、Zotero、Readwise 也尚无受控真实证据，因此 P2-D/发布总检查点不变；formatter 不再是发布阻塞。
...
- schema v2 契约验收结束时 `policySetVersion=2`，九种策略全部禁用且四组授权数组为空；2026-08-17 的 qBittorrent canary 使用版本 3 的单 provider 最小授权，完成后发布版本 4 恢复九类全禁用。2026-08-18 的 qBittorrent 公网矩阵因两次失败关闭复跑依次使用版本 7/8、9/10、11/12 的启用/禁用对。Webhook 首轮因验收器模式断言错误使用版本 13/14 并自动恢复，修正后成功矩阵使用版本 15/16；最终版本 16 九类全禁用、ACTIVE 为空。版本 1 是不含真实 provider、凭据或可达目标的临时 advanced-only 契约探针；所有真实启用版本只包含对应 canary 的最小非秘密授权，不含凭据、完整 Webhook 路径或条目内容。
...
- 2026-08-17 策略恢复版本 4 后，真实 exporter 返回不可重试 `AccessDenied`，provider 对象仍为 0。随后本机 target 按 marker 1→0 降级、LenxTool DPAPI 测试凭据删除，LenxTool 与 qBittorrent 进程均优雅退出。qBittorrent 自身隔离 profile 保留，便于后续受控复验。
- 2026-08-18 生产 `TorrentFileFetcher` 从 Ubuntu 官方 HTTPS/443 取得 508,158 字节 `.torrent`，真实经过 DNS Fake-IP 分类、地址 pin、禁代理/跳转/Cookie/解压、MIME、2 MiB 正文上限和 bencode/info-hash 校验；真实 exporter 完成首次写入、同请求重放、info-hash/category/stopped 复查及精确删除，不存在的公网子域被映射为可重试 `DestinationUnavailable`。
- 独立脱敏观测确认 WebAPI 对合法文件返回 200；同一已存在 hash 的重复文件返回 409，原对象 `added_on` 不变；远程 Ubuntu `.torrent` URL 返回 202，随后收敛为预期 hash/category/stopped 对象并精确删除。qBittorrent 5.2.3 源码显示有效 magnet 已能解析 hash，当前产品也只发送可解析 magnet 或已下载文件，因此该 202 是 provider URL 型异步行为与产品防御性分支的真实佐证，不冒充产品公网文件路径。
- 首轮诊断把 provider 启动放在已经结束的命令会话，约 156 秒后执行环境正常回收 qBittorrent，导致验收器清理连接被拒；qBittorrent 自身日志为正常保存/退出，Windows 只记录验收器未处理异常。改为单一会话执行“启动→验收→精确清理→策略撤销→WebAPI 关闭→凭据回收”后完整通过。
- 最终策略版本 12 九类全禁用、ACTIVE 0；撤销后 exporter 返回 `AccessDenied` 且 provider 对象为 0。target marker 0、LenxTool DPAPI 测试凭据删除，qBittorrent/LenxTool 进程、47891/6272 监听和隔离下载目录文件均为 0，Worker `/health` 为 200。请求级秘密、完整响应与本地证据不进入仓库。

## Desktop v2 / Webhook 真实 canary

- 固定源码候选为 `84945757f66a0e01e855ab00a44173e8236cbbd0`。一次性接收器通过随机 Quick Tunnel HTTPS 主机与路径暴露，只在内存保存计数和 `eventId → SHA-256(body)`，不持久化正文；控制令牌与 HMAC secret 仅存在于单次会话环境和已删除的临时文件。
- 真实 Release Desktop 从生产 Worker 恢复 ADMIN 会话，以版本 15 仅启用 Webhook 公网主机；本机目标按 marker 0→DPAPI→marker 1 激活。健康探针返回 `Healthy`，实际经过 ACTIVE 策略、公网 DNS 全地址分类、地址 pin、禁代理/跳转/Cookie/解压、DPAPI 和 OPTIONS 能力检查。
- 固定 v1 JSON 首写与同一请求重放只产生 1 个业务事件，接收器验证 HMAC-SHA256 覆盖实际 UTF-8 正文；能力声明缺失时 exporter 返回不可重试 `AccessDenied` 且 POST 增量为 0。ack 不匹配映射为可重试 `DestinationUnavailable`，恢复正常后同一键只计重复、不产生第二个业务事件；503 同样可重试，恢复后只创建 1 个事件。整个矩阵恰好产生 3 个预期业务事件。
- 首轮在能力缺失场景后停止，根因是验收器的统计断言把接收器模式硬编码为 `normal`，而实际正确模式是 `capability-missing`；真实请求计数和产品错误分类均正确。`finally` 已将版本 14 恢复全禁用并删除 target/DPAPI。修正为按场景核对模式、补充安全阶段诊断后，从空基线完整复跑通过。
- 版本 16 撤销策略后，真实 exporter 返回不可重试 `AccessDenied`，接收器 OPTIONS/POST 增量均为 0。最终版本 16 九类全禁用、ACTIVE 0，Webhook target 与 DPAPI 测试凭据不存在，临时接收器、Quick Tunnel、helper 进程、监听和秘密文件均为 0。没有秘密、正文、完整响应或请求级证据进入 tracked diff。

## 下一步与完成条件

本报告证明的是“源码、假 HTTP 契约、本地安全边界和 qBittorrent/Webhook 全部真实 canary 完成”，不等同于其余外部适配器或正式安装包已发布。下一轮必须使用固定发布候选提交，按 [`P2-D 执行手册`](plans/RSS_P2_VIEWS_INTEGRATIONS.md#p2-d-执行手册) 完成：

1. 准备受控 Readeck 实例、Bearer token 和可清理测试书签，登记版本、精确 endpoint、时间窗与回滚负责人；执行标签查询、首次创建、重放、归档、暂时故障、未知回执和撤权矩阵。
2. 准备受控 Outline 实例、个人 API key 和专用测试 collection UUID；执行 collection 身份、`publish=false` 个人草稿、重复更新、目标切换、暂时故障、未知回执和撤权矩阵。
3. Readeck/Outline 完成后，依次补 Eagle 独立测试资源库、Zotero 个人测试库和 Readwise Reader 测试账号的健康、首写、重放、暂时故障、清理与撤权证据。
4. 每轮基于当前版本 16 的九类全禁用基线只启用一个真实对象，完成后恢复全禁用并清理 marker/测试对象；秘密只通过密码管理器/运行时输入，不进入日志、截图、Issue 或仓库。
5. formatter 历史基线已经独立关闭；全部真实矩阵通过后，按 [`RELEASE_GUIDE.md`](RELEASE_GUIDE.md) 生成并验证签名安装包、便携包、更新清单和 SHA256，再完成 Windows 10/11 x64 发布矩阵，并在发布候选上重跑 formatter 与完整门禁。

P2-D 关闭前不得宣称端到端生产验收通过；P2-D 关闭后仍需完成生产发布标签、GitHub Release 和跨物理机升级矩阵。

**2026-08-10 历史门禁：** P2-16～P2-19 未接通入口门禁先以失败测试证明个人通用设置曾同时暴露 Readwise、Readeck、Outline、qBittorrent 与 Webhook 5 项，随后改为显式生产 allowlist：当时只显示已注册 exporter/健康探针且固定官方目标的 Readwise；四种占位类型的程序化赋值与旧持久化配置均回到安全默认值，不能进入 TargetId、DPAPI 凭据或连接测试流程。升级前配置只留下非秘密清理指针；ViewModel 不按旧槽位取值，旧值不会返回界面或交给探针/exporter。直接删除当前记录会以“先写安全 kind、再写目标”的可重放顺序规范化仍匹配的旧设置，逐键写入失败后仍可重试，不会让清理提示复生。历史未引用槽位可由用户按原类型和 TargetId 通过 delete-only 入口定向删除；哈希槽名不能反推出遗忘的标识。生产存储使用共享 DPAPI blob，刷新当前 Readwise presence 或删除槽位时可能在存储层解密整个 blob，因此本门禁不宣称进程从不解密旧值，只保证旧值不离开存储层、不进入 UI、探针、exporter 或网络。ViewModel/布局聚焦 25/25、App 非 WPF 全量 508/508、Core 全量 202/202 通过；2026-08-10 当轮的 10 个 WPF 类逐进程串行为 13/14，唯一失败是未修改的 `SelectionControlsWpfRuntimeTests` 第 124 行 `CalendarAutomationPeer.GetChildrenCore`，`ToolsWpfRuntimeTests` 等其余类通过。解决方案 Release build 为 0 警告/0 错误，NuGet 在线漏洞审计为 0。该历史门禁不代表当前实现状态。

2026-08-13 关闭了上述 `SelectionControlsWpfRuntimeTests` 门禁。失败先行在当前主线稳定复现：测试打开原生 `DatePicker` 后只应用外层 `Calendar` 模板，随即强制 `UpdateLayout`；此时内层 `CalendarItem` 尚未经过自然 Loaded/布局流程，而 WPF `CalendarAutomationPeer` 会直接为 Previous/Header/Next 三个模板部件创建 peer，因而把空元素传入框架 API。修复只调整真实 WPF 验收宿主：不手动应用任何模板，等待生产 DatePicker 的嵌套控件及三个导航部件自然加载，再显式调用 Calendar peer 的 `GetChildren()`，核对三个导航 peer 和包含日期单元的完整子树。生产 XAML、键盘、原生 Automation Peer 和视觉行为均未改动。精确用例 1/1，并以独立进程连续 10/10；10 个 WPF 类逐进程串行共 14/14。解决方案首轮只有未改动的 1,000 条混合分页性能用例以 2.092 秒越过 2 秒预算，精确隔离为 636 ms，随后 Infrastructure 独立全量 763/763 通过；最终无构建全解决方案复跑 App 522/522、Core 202/202、Infrastructure 763/763，共 1487/1487。未放宽阈值或修改性能代码。Release build 为 0 警告/0 错误；Worker、NuGet 与 npm 审计未重跑。
...
Independent-01 首轮 App 全量同进程运行结果为 502/508；6 项失败均落在既有 `CalendarAutomationPeer.GetChildrenCore` 空元素调用栈，未经过 JSON Diff。该任务当轮的 10 个 WPF 类逐进程串行为 13/14，Feed Audio、Discovery、Thumbnail、Picture、Timeline、Video、ViewSwitching、Tools 和 SmoothWheel 类均通过；唯一失败是未改动的 `SelectionControlsWpfRuntimeTests` 第 124 行同一 Calendar AutomationPeer 调用栈。本任务未修改 Infrastructure、Worker、数据库 schema 或依赖，因此没有把 2026-08-08 的对应结果描述成本轮重跑。
...
当前本轮 Release 结果为 Core 177/177、Infrastructure 714/714、App 非 WPF-runtime 399/399，共 1290 个未阻断 .NET 用例通过；滚动专项 26/26（纯逻辑 21/21、真实 WPF 5/5）通过，全解决方案 build 为 0 警告/0 错误。完整 App 运行时矩阵为 404/408，余下 4 项均在既有日历 `CalendarAutomationPeer` 创建空元素处失败，干净暂存快照可复现且调用栈不经过滚动实现，因此记录为既有 WPF Automation 宿主门禁，而不是本次滚动回归。最新 Release 已由用户在“每日早报”实际体验并确认流畅。Worker 本轮未修改也未重跑，沿用 2026-08-03 的 workerd/D1 Vitest 78/78 与 strict typecheck 证据；本轮未修改 NuGet/npm 依赖。
...
P2-08 新增 Core 12 项、Infrastructure 7 项、App/WPF 8 项与 Worker 4 项定向场景，并扩展 migration、P1 D1 隐私哨兵和 DI 回归。Core 覆盖九种封闭类型、精确 IDN 主机规范化、不可变输入、空集合默认关闭、重复/未知类型，以及协议、端口、路径、通配符、IP、localhost 和 `.local` 拒绝；公共策略/健康结果不携带凭据、供应商响应或异常详情。Worker 真实 workerd/D1 覆盖 0010 无私人字段表结构、admin 整集发布、user ACTIVE 只读/ALL 与写入 403、鉴权先于正文、条件版本、幂等重放、不可变快照和最小审计。客户端覆盖 Worker 严格映射与只含共享字段的 PUT、哈希 DPAPI 槽位、缺凭据/停用/私网 DNS 零探针调用、超时、调用方取消、30 秒冷却、封闭脱敏状态，以及生产 DI 零探针。App 覆盖普通用户零 ALL/写入、管理员精确主机整集发布、个人目标只写非敏感 SQLite 设置、凭据保存后清空、目标地址失效后仍可撤销凭据、独立管理页/设置卡片的滚动、键盘原生控件和自动化名称。完整门禁为 Core 173/173、Infrastructure 397/397、App/WPF 312/312，共 882/882；Worker 74/74、strict typecheck 与 Release build 0 警告/0 错误。Wrangler 尝试写沙箱外调试日志时报告 EPERM，但测试进程以退出码 0 完成 12/12 文件和 74/74 场景。
...
全局滚轮已切换为 `TwilightLemon/FluentScrollViewer` 的原版双模式运动核心。标准鼠标使用 2.0 速度倍率、0.92 摩擦和 144 Hz 时间基准，连续输入直接代数叠加速度，因此反向输入先抵消旧动量；高分辨率触控板按 0.5 插值系数从每次输入到达时的真实偏移追随目标。每个显示帧读取真实 `VerticalOffset` 后直接调用 `ScrollToVerticalOffset`，不再使用解析积分、固定落点或跨刷新率落点校正。外围仍保留嵌套边界冒泡；Shift 横向意图、Windows 禁用滚轮/客户端动画和“减少动画”会交回原生 WPF。

热点、每日早报和虚拟列表均按显示帧提交真实逻辑偏移，内容 `RenderTransform` 不再参与滚轮动画。每日早报的延迟正文协调器按四分之一屏且最高 120 px 的跨会话累计位移统一刷新，并对延迟到会话结束后的 `ScrollChanged` 使用同一阈值，避免恢复逐帧全文扫描；结构或尺寸变化仍会强制刷新。拖动、键盘、触控和程序化定位会取消残余动量，卸载会清空速度与输入分类状态；回到顶部保持立即提交。

滚动模型专项 21/21 通过，覆盖上游参考帧摩擦、无帧间隔上限、触控板插值、同向累速、反向抵消、边界持续衰减、宿主动效闸门和正文刷新阈值；真实 WPF 专项 5/5 通过，覆盖多帧逻辑偏移、连续输入复用会话、无内容 `RenderTransform`、普通/早报一致性、直接输入接管、卸载重载清状态及长文扫描节流。Release 构建为 0 警告/0 错误；用户已在最新 Release 每日早报实际体验并确认滚动流畅。上游 MIT 原文随源码和构建输出保留，Worker 与依赖未修改。
...
P1-07 新增 25 个 Infrastructure 场景：12 组独立文章 fixture 覆盖 `article`/`main`/内容容器、中文、列表、引用、相对链接、延迟图片、噪声布局、畸形 HTML、元数据和提示注入文本；其余场景覆盖 GB18030、脚本/样式/表单/iframe/危险协议净化、空正文警告、DOM 深度、正文/块上限、最终 URL、固定 DNS 地址、私网 DNS/重定向阻断、响应大小、调用方取消、总超时和同主机并发 1。Core 契约不暴露解析器类型，组合根测试确认 `IArticleContentExtractor` 可解析。`dotnet list LenxTool.slnx package --vulnerable --include-transitive` 对包括 `HtmlAgilityPack 1.12.4` 在内的全部项目报告 0 个已知易受攻击包。
